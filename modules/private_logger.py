...
def log(img, metadata, metadata_parser: MetadataParser | None = None, output_format=None, task=None, persist_image=True) -> str:
    ...
    if output_format == OutputFormat.PNG.value:
        if parsed_parameters != '':
            ...
            pnginfo.add_text('CensuradoIA_scheme', metadata_parser.get_scheme().value)
        else:
            pnginfo = None
    ...
    begin_part = f"<!DOCTYPE html><html><head><title>CensuradoIA Log {date_string}</title>{css_styles}</head><body>{js}<p>CensuradoIA Log {date_string} (private)</p>\n<p>Metadata is embedded if enabled in the config or developer debug mode. You can find the information for each image in line Metadata Scheme.</p><!--CensuradoIA-log-split-->\n\n"
    end_part = f'\n<!--CensuradoIA-log-split--></body></html>'

    middle_part = log_cache.get(html_name, "")

    if middle_part == "":
        if os.path.exists(html_name):
            existing_split = open(html_name, 'r', encoding='utf-8').read().split('<!--CensuradoIA-log-split-->')
