...
class A1111MetadataParser(MetadataParser):
    ...
    def to_json(self, metadata: str) -> dict:
        ...
        *lines, lastline = metadata.strip().split("\n")
        ...
        for line in lines:
            line = line.strip()
            if line.startswith(f"{self.CensuradoIA_to_a1111['negative_prompt']}:"):
                done_with_prompt = True
                line = line[len(f"{self.CensuradoIA_to_a1111['negative_prompt']}:"):].strip()
            if done_with_prompt:
        ...
        for k, v in re_param.findall(lastline):
            try:
                ...
                if m is not None:
                    ...
                    data[list(self.CensuradoIA_to_a1111.keys())[list(self.CensuradoIA_to_a1111.values()).index(k)]] = v
            except Exception:
        ...
        if 'raw_prompt' in data:
            ...
            if metadata_prompt != raw_prompt and modules.sdxl_styles.CensuradoIA_expansion not in found_styles:
                found_styles.append(modules.sdxl_styles.CensuradoIA_expansion)
    ...
    def to_string(self, metadata: dict) -> str:
        ...
        generation_params = {
            self.CensuradoIA_to_a1111['steps']: self.steps,
            self.CensuradoIA_to_a1111['sampler']: sampler,
            self.CensuradoIA_to_a1111['seed']: data['seed'],
            self.CensuradoIA_to_a1111['resolution']: f'{width}x{height}',
            self.CensuradoIA_to_a1111['guidance_scale']: data['guidance_scale'],
            self.CensuradoIA_to_a1111['sharpness']: data['sharpness'],
            self.CensuradoIA_to_a1111['adm_guidance']: data['adm_guidance'],
            self.CensuradoIA_to_a1111['base_model']: Path(data['base_model']).stem,
            self.CensuradoIA_to_a1111['base_model_hash']: self.base_model_hash,

            self.CensuradoIA_to_a1111['performance']: data['performance'],
            self.CensuradoIA_to_a1111['scheduler']: scheduler,
            self.CensuradoIA_to_a1111['vae']: Path(data['vae']).stem,
            # workaround for multiline prompts
            self.CensuradoIA_to_a1111['raw_prompt']: self.raw_prompt,
            self.CensuradoIA_to_a1111['raw_negative_prompt']: self.raw_negative_prompt,
        }

        if self.refiner_model_name not in ['', 'None']:
            generation_params |= {
                self.CensuradoIA_to_a1111['refiner_model']: self.refiner_model_name,
                self.CensuradoIA_to_a1111['refiner_model_hash']: self.refiner_model_hash
            }

        for key in ['adaptive_cfg', 'clip_skip', 'overwrite_switch', 'refiner_swap_method', 'freeu']:
            if key in data:
                generation_params[self.CensuradoIA_to_a1111[key]] = data[key]

        if len(self.loras) > 0:
            lora_hashes = []
            lora_weights = []
            for index, (lora_name, lora_weight, lora_hash) in enumerate(self.loras):
                # workaround for CensuradoIA not knowing LoRA name in LoRA metadata
            ...
            generation_params[self.CensuradoIA_to_a1111['lora_hashes']] = lora_hashes_string
            generation_params[self.CensuradoIA_to_a1111['lora_weights']] = lora_weights_string

        generation_params[self.CensuradoIA_to_a1111['version']] = data['version']

        if modules.config.metadata_created_by != '':
            generation_params[self.CensuradoIA_to_a1111['created_by']] = modules.config.metadata_created_by
...
class CensuradoIAMetadataParser(MetadataParser):
    def get_scheme(self) -> MetadataScheme:
...
def get_metadata_parser(metadata_scheme: MetadataScheme) -> MetadataParser:
    match metadata_scheme:
        case MetadataScheme.FOOOCUS:
            return CensuradoIAMetadataParser()
...
def read_info_from_image(file) -> tuple[str | None, MetadataScheme | None]:
    ...
    metadata_scheme = items.pop('CensuradoIA_scheme', None)
...
def get_exif(metadata: str | None, metadata_scheme: str):
    ...
    # 0x0131 = Software
    exif[0x0131] = 'CensuradoIA v' + CensuradoIA_version.version
    # 0x927C = MakerNote
