...
def worker():
    ...
    from modules.sdxl_styles import apply_style, get_random_style, CensuradoIA_expansion, apply_arrays, random_style_name
    ...
    def save_and_log(async_task, height, imgs, task, use_expansion, width, loras, persist_image=True) -> list:
        ...
        for x in imgs:
            d = [('Prompt', 'prompt', task['log_positive_prompt']),
                 ...
                 ('CensuradoIA V2 Expansion', 'prompt_expansion', task['expansion']),
                 ('Styles', 'styles',
                  str(task['styles'] if not use_expansion else [CensuradoIA_expansion] + task['styles'])),
            ...
            d.append(('Version', 'version', 'CensuradoIA v' + CensuradoIA_version.version))
    ...
    def process_prompt(async_task, prompt, negative_prompt, base_model_additional_loras, image_number, disable_seed_increment, use_expansion, use_style,
        ...
        if use_expansion:
            ...
            for i, t in enumerate(tasks):
                ...
                progressbar(async_task, current_progress, f'Preparing CensuradoIA text #{i + 1} ...')
    ...
    def handler(async_task: AsyncTask):
        ...
        if CensuradoIA_expansion in async_task.style_selections:
            use_expansion = True
            async_task.style_selections.remove(CensuradoIA_expansion)
        else:
            use_expansion = False
