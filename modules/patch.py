...
def patched_load_models_gpu(*args, **kwargs):
    ...
    if moving_time > 0.1:
        print(f'[CensuradoIA Model Management] Moving model(s) has taken {moving_time:.2f} seconds')
    return y
            ...
            for path in list(args) + list(kwargs.values()):
                if isinstance(path, str):
                    if os.path.exists(path):
                        ...
                        if os.path.exists(path):
                            os.remove(path)
                        exp += f'CensuradoIA has tried to move the corrupted file to {corrupted_backup_file} \n'
                        exp += f'You may try again now and CensuradoIA will download models again. \n'
